...
std::string DataBase::findOne(const std::string type, std::string val, int index)
{
    try
    {
        ...
        {
            if(line.find(val) != std::string::npos) // ?õê?ïò?äî row Ï∞æÍ∏∞
            {
...
// Î≠? ÎßåÏïΩ?óê ?ú†??? Î°úÍ∏¥?ï¥?Ñú id pw Î¥? id ?ûà?äîÏß? Î≥? ?ñÑ ???Í±∏Î°ú db.FindOne("student", "exid2", 2) Î°? ?ïÑ?ù¥?îî ?ùºÏπòÌïòÍ≥?, db.FindOne("student", "expass", 3) Î∞õÏïÑ?Ñú ?ôï?ù∏.
...
void DataBase::update(const std::string type, std::string primaryKey, std::string content, int index)
{
    try
    {
        std::string directory = findDB(type);
        std::ifstream fin(directory);
        std::ofstream fout("temp.txt");  // ?ùº?ãú????û•?ö©
        ...
        try
        {
            if (fin.is_open() && fout.is_open())
            {
                ...
                while (std::getline(fin, line))
                {
                    ...
                    if (words[0] == primaryKey) // ?ù¥Í±∏Î°ú ?Öå?ù¥Î∏? & row Íµ¨Î∂Ñ
                    {
                        // string ?†ú?ûë
                        int i;
                        std::string newData = "";

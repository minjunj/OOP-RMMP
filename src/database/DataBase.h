...
public:
    ...
    /*
        ?ç∞?ù¥?Ñ∞ Î≤†Ïù¥?ä§?óê ?ÇΩ?ûÖ?ïò?äî ?ï®?àò
        insert( _JSON?úºÎ°? ????û•?ïú data, ????û•?úÑÏπ? : admin, student, room)
    */
    template <typename T>
    ...
    */
    std::vector<std::unique_ptr<Student>> student_JSON(int code = 0, const std::string& name = "NULL", const std::string& id = "NULL", const std::string& pw = "NULL",
            const std::string& class_ = "NULL", const std::string& room = "NULL", const bool gender = true, const std::string& mateID = "NULL"); // ?Ñ±Î≥? Í∏∞Î≥∏ Í∞íÏ?? ture


    // Admin(std::string adminID, string userName, string userID, string userPW);
    /* ?ñ¥?ìúÎØ? ?ç∞?ù¥?Ñ∞ ?ûÖ?†•?ïò?äî ?ï®?àò*/
